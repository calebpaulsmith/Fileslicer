# RAG Export Notes — longsrc

- **Target:** rag
- **Mode:** balanced
- **Documents:** 1
- **Estimated total tokens:** 331
...
Chunks are split greedily by paragraph against the per-chunk token budget (or at headings when the heading strategy is selected). Chunk size, strategy, overlap between adjacent chunks, a minimum chunk size that merges tiny chunks, and sentence-level splitting of oversize lines are all configurable in the chunk review screen and saved profiles.
