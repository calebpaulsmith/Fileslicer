...
Chunks are split greedily by paragraph against the per-chunk token budget (or at headings when the heading strategy is selected). Chunk size, strategy, overlap between adjacent chunks, and a minimum chunk size that merges tiny chunks are all configurable in the chunk review screen and saved profiles. If you want sentence-level splitting, pre-process `chunks.jsonl` before embedding.
